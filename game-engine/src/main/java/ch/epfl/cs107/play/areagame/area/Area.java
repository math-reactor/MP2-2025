...
public abstract class Area implements Playable, Graphics, PauseMenu.Pausable {
    ...
    public final void purgeAreaCellsFrom(Interactable entity) {
        List<DiscreteCoordinates> coords = new LinkedList<>();
        ...
        areaBehavior.leave(entity, coords);
    }
    ...
    //TODO make acoustics (.bip()) in its own function ?
    @Override
    public void draw(Canvas canvas) {
        if (paused && menu != null) {
            //TODO draw menu here instead of update()?
            return;
        }
        for (Actor actor : actors) {
            actor.bip(window);
